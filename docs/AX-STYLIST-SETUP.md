...
1. Run [the base migration](../supabase/migrations/20260915_ax_stylist.sql), then [usage and advanced budget](../supabase/migrations/20260915235149_ax_stylist_usage_and_model_budget.sql), then [quota alerts](../supabase/migrations/20260924_stylist_quota_alerts.sql), then [Data API grant hardening](../supabase/migrations/20260924_supabase_data_api_grants.sql).
2. Confirm `ax_stylist_profiles`, `ax_stylist_limits` and `ax_stylist_usage` have Row Level Security enabled. `anon` and `authenticated` must have no access. The server-only service-role/secret key accesses them through explicit Supabase Data API grants. Do not copy Supabase's public-table example grants onto these private tables.
