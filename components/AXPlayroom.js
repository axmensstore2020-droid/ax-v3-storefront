...
export default function AXPlayroom({initialStatus=null,launchMode=false}){
 ...
 const resultKicker=result?.winner==='O'?(launchMode?'10% launch reward':'10% unlocked'):result?.winner==='draw'?(finalDraw?'Good game':'Bonus round unlocked'):'Good game';
 const resultCopy=result?.winner==='O'?(launchMode?'Your launch-day 10% reward is secured. Save the code for Sunday.':'Your verified 10% code is ready for your next AX order.'):result?.winner==='draw'?(finalDraw?'That was your bonus round. AX Playroom returns tomorrow.':(launchMode?'Your draw unlocked one Bonus Round — one last shot at the launch-day 10%.':'No discount used. Your draw unlocked one Bonus Round — one last shot at 10%.')):(launchMode?'No reward yet. Come back for another shot before launch.':'No penalty. The Playroom will be back tomorrow.');
