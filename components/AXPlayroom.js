...
export default function AXPlayroom(){
 ...
 const [settlement,setSettlement]=useState({state:'idle',outcome:null,rewardCode:null,rewardEndsAt:null,nextEligibleAt:null,error:''});
 const cells=useRef([]);
 ...
 async function settle(nextMoves,nextOutcome){
  ...
  setSettlement({state:'loading',outcome:expected,rewardCode:null,rewardEndsAt:null,nextEligibleAt:null,error:''});
  try{
   ...
   setSettlement({state:'ready',outcome:data.outcome,rewardCode:data.rewardCode||null,rewardEndsAt:data.rewardEndsAt||null,nextEligibleAt:data.nextEligibleAt||null,error:''});
  }catch(error){
 ...
 const resultKicker=result?.winner==='O'?'10% unlocked':result?.winner==='draw'?'Another round unlocked':'Good game';
 const resultCopy=result?.winner==='O'?'Your verified 10% code is ready for your next AX order.':result?.winner==='draw'?'No discount used. A draw unlocks one more round right now.':'No penalty. The Playroom will be back tomorrow.';
 const miniTitle=phase==='result'&&settlement.state==='loading'?'Checking your result':phase==='result'&&settlement.outcome==='win'?'10% reward ready':phase==='result'&&settlement.outcome==='draw'?'Another round unlocked':phase==='result'&&settlement.outcome==='loss'?'Good game':'Play AX XO';
 const miniCopy=phase==='result'&&settlement.state==='loading'?'Tap to reopen':phase==='result'&&settlement.outcome==='win'?'Tap to copy your code':phase==='result'&&settlement.outcome==='draw'?'Tap to play again':phase==='result'&&settlement.outcome==='loss'?'Tap to view your result':'Win 10% off';
