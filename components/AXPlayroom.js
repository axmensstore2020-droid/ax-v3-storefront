...
export default function AXPlayroom({initialStatus=null,launchMode=false}){
 ...
 return <>
  ...
  {open&&<div className="ax-xo-backdrop" role="presentation"><section className={'ax-xo-modal'+(motionOn?'':' reduce-motion')+(launchMode?' launch-mode':'')} role="dialog" aria-modal="true" aria-labelledby="ax-xo-title">
   ...
   <footer className="ax-xo-footer"><span>{launchMode?'✣ Beat AX before we go live.':'✣ Good style. Great moves.'}</span><span>{launchMode?'Best of 3 · First to 2 · Win 10%':'Win 10% · One draw bonus · Lose nothing.'}</span><a href={launchMode?'/coming-soon/launch.ics':'/products'} onClick={minimizeGame}>{launchMode?'Mark your calendars':'Explore AX'} <span aria-hidden="true">↗</span></a></footer>
