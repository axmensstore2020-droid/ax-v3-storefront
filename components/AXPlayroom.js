...
export default function AXPlayroom({initialStatus=null,launchMode=false}){
 ...
 return <>
  ...
  {open&&<div className="ax-xo-backdrop" role="presentation"><section className={'ax-xo-modal'+(motionOn?'':' reduce-motion')+(launchMode?' launch-mode':'')} role="dialog" aria-modal="true" aria-labelledby="ax-xo-title">
   ...
   <main className="ax-xo-experience">
    ...
    <section className="ax-xo-play-side" aria-label="Play tic-tac-toe with AX"><div className="ax-xo-play-shell"><div className="ax-xo-game-top"><span>You × AX</span><span>{bonusRound?'BONUS ROUND':rounds?'Round '+String(rounds).padStart(2,'0'):'A friendly match'}</span></div><div className="ax-xo-players" data-turn={turn}><span className="ax-xo-turn-track" aria-hidden="true"/><div className={'ax-xo-player'+(turn==='O'?' active':'')}><span className="ax-xo-player-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="7.5"/></svg></span><span>You<small>Always O</small></span><i/></div><div className={'ax-xo-player ax'+(turn==='X'?' active':'')}><span className="ax-xo-player-icon"><svg viewBox="0 0 24 24"><path d="m5 5 14 14M19 5 5 19"/></svg></span><span>AX<small>Always X</small></span><i/></div></div>
     ...
     <div className="ax-xo-stage-wrap">
      ...
      {phase==='result'&&<section className={'ax-xo-stage ax-xo-result-stage outcome-'+(result?.winner||'')+(launchMode&&result?.winner==='O'?' premium-win':'')}>{result?.winner==='O'&&!launchMode&&<div className="ax-xo-confetti" aria-hidden="true">{Array.from({length:22},(_,index)=><i key={index}/>)}</div>}<div className="ax-xo-result-badge" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M8 3h8v6a4 4 0 0 1-8 0zM8 5H4v2a4 4 0 0 0 4 4m8-6h4v2a4 4 0 0 1-4 4m-4 2v5m-4 3h8m-10 0h12"/></svg></div><p className="ax-xo-result-kicker">{resultKicker}</p><h2>{resultTitle}</h2><p className="ax-xo-result-copy">{resultCopy}</p>
       {result?.winner==='O'&&<div className={'ax-xo-ticket '+(settlement.state==='error'?'has-error':'')}><div className="ax-xo-ticket-percent"><b>10</b><small>%</small><span>AX reward</span></div><div className="ax-xo-ticket-info"><strong>{settlement.state==='ready'&&settlement.rewardCode?settlement.rewardCode:settlement.state==='error'?'REWARD PENDING':'CREATING CODE…'}</strong><p>{settlement.state==='error'?settlement.error:(launchMode?'Applicable at launch checkout · Sunday 12 PM IST · launch day only':'Single use · valid 7 days · non-stackable')}</p></div><button type="button" className="ax-xo-copy" disabled={!settlement.rewardCode} onClick={copyCode} aria-label="Copy 10 percent reward code"><svg viewBox="0 0 24 24"><rect x="8" y="8" width="12" height="13" rx="2"/><path d="M16 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h3"/></svg></button></div>}
       {settlement.state==='error'&&<button type="button" className="ax-xo-retry-reward" onClick={retrySettlement}>Retry verification</button>}
       <div className="ax-xo-result-actions">{result?.winner==='draw'&&settlement.bonusAvailable?<button type="button" className="solid" disabled={settlement.state!=='ready'} onClick={()=>resetRound(true)}>Play bonus round <span aria-hidden="true">↗</span></button>:result?.winner==='draw'?<button type="button" className="solid" disabled={settlement.state!=='ready'} onClick={minimizeGame}>{launchMode?'Back to launch':'Back to AX'} <span aria-hidden="true">↗</span></button>:result?.winner==='O'?<button type="button" className="solid" disabled={!settlement.rewardCode} onClick={copyCode}>Copy 10% code <span aria-hidden="true">↗</span></button>:<button type="button" className="solid" onClick={minimizeGame}>{launchMode?'Back to launch':'Back to AX'} <span aria-hidden="true">↗</span></button>}{result?.winner==='O'&&launchMode?<a className="ax-xo-calendar-action" href="/coming-soon/launch.ics">Mark your calendars <span aria-hidden="true">↗</span></a>:<button type="button" onClick={()=>setPhase('review')}>See final board</button>}</div><p className="ax-xo-result-footnote">{result?.winner==='O'?(launchMode?'Limited stock. Your verified 10% discount applies at checkout on launch day.':'One code per verified win. The Playroom returns in 7 days.'):result?.winner==='draw'?(settlement.bonusAvailable?'One draw unlocks one Bonus Round only.':'Bonus Round complete. Your next Playroom unlocks tomorrow.'):(launchMode?'No discount, no penalty. Try again before launch.':'No discount, no penalty. Your next round unlocks tomorrow.')}</p></section>}
     </div>
   ...
   <footer className="ax-xo-footer"><span>{launchMode?'✣ Beat AX before we go live.':'✣ Good style. Great moves.'}</span><span>{launchMode?'Beat AX · Win 10% · See you Sunday':'Win 10% · One draw bonus · Lose nothing.'}</span><a href={launchMode?'/coming-soon/launch.ics':'/products'} onClick={minimizeGame}>{launchMode?'Mark your calendars':'Explore AX'} <span aria-hidden="true">↗</span></a></footer>

   {showRules&&<div className="ax-xo-rules-backdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target)setShowRules(false);}}><section className="ax-xo-rules" role="dialog" aria-modal="true" aria-labelledby="ax-xo-rules-title"><button type="button" onClick={()=>setShowRules(false)} aria-label="Close how to play"><svg viewBox="0 0 24 24"><path d="m6 6 12 12M18 6 6 18"/></svg></button><p className="ax-xo-eyebrow">A quick breather</p><h2 id="ax-xo-rules-title">Three makes a win.</h2><ol><li>You’re always <strong>O</strong>. AX is always <strong>X</strong>. Choose who makes the opening move.</li><li>Tap an empty square. Get three of your marks in a row, column, or diagonal.</li><li><strong>Win:</strong> get one verified 10% off code. <strong>First draw:</strong> unlock one Bonus Round immediately. If the Bonus Round also draws, the Playroom returns tomorrow. <strong>AX wins:</strong> lose nothing and come back tomorrow.</li></ol><p>{launchMode?'Launch rewards are single-use, activate Sunday at 12:00 PM IST, expire at the end of launch day and cannot be combined with other discounts.':'Win codes are single-use, valid for 7 days and cannot be combined with other discounts. A verified win also pauses the Playroom for 7 days.'}</p><p className="demo ax-xo-live-note">Results are verified by AX before a reward code is issued. No purchase is required to play.</p></section></div>}
