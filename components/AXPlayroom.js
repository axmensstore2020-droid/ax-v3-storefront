...
export default function AXPlayroom({initialStatus=null,launchMode=false}){
 ...
 async function start(mark){
  ...
  try{
   ...
   if(launchMode)trackLaunchEvent('launch_game_start',{metadata:{first:mark,series_you:Number(settlement.series?.you)||0,series_ax:Number(settlement.series?.ax)||0}});
  }catch(error){
 ...
 async function settle(nextMoves,nextOutcome){
  ...
  try{
   ...
   if(launchMode&&data.series){
    setScores({O:Number(data.series.you)||0,X:Number(data.series.ax)||0,draw:Number(data.series.draws)||0});
    const roundKey='round-'+String(data.series.rounds||0);
    trackLaunchEvent('launch_round_complete',{onceKey:roundKey,metadata:{outcome:data.outcome,you:Number(data.series.you)||0,ax:Number(data.series.ax)||0,draws:Number(data.series.draws)||0,rounds:Number(data.series.rounds)||0}});
    if(data.matchComplete){
      const matchKey=data.rewardCode||data.nextEligibleAt||('match-'+String(data.series.rounds||0));
      trackLaunchEvent('launch_match_complete',{onceKey:matchKey,metadata:{outcome:data.matchOutcome||'',you:Number(data.series.you)||0,ax:Number(data.series.ax)||0,draws:Number(data.series.draws)||0,rounds:Number(data.series.rounds)||0}});
      if(data.matchOutcome==='win'){
        trackLaunchEvent('launch_match_win',{onceKey:matchKey,metadata:{you:Number(data.series.you)||0,ax:Number(data.series.ax)||0,rounds:Number(data.series.rounds)||0}});
        if(data.rewardCode)trackLaunchEvent('launch_reward_issued',{onceKey:data.rewardCode,metadata:{launch_day:true}});
      }
    }
   }
 ...
 function openGame(){setVisible(true);setOpen(true);if(launchMode)trackLaunchEvent('launch_playroom_open',{metadata:{source:'minimized'}});}
 ...
 return <>
  ...
  {open&&<div className="ax-xo-backdrop" role="presentation"><section className={'ax-xo-modal'+(motionOn?'':' reduce-motion')+(launchMode?' launch-mode':'')} role="dialog" aria-modal="true" aria-labelledby="ax-xo-title">
   <header className="ax-xo-topbar"><div className="ax-xo-brand"><Brand/><span>{launchMode?<>Website launch<br/>01 OCT · 4 PM</>:<>A little<br/>off the clock.</>}</span></div><div className="ax-xo-top-actions"><span className="ax-xo-edition"><i/>{launchMode?'Playroom / Launch':'Playroom / Live'}</span><button type="button" className="ax-xo-icon-button" aria-label={motionOn?'Turn animations off':'Turn animations on'} aria-pressed={motionOn} onClick={()=>setMotionOn(value=>!value)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h3l3-7 3 14 3-7h2"/></svg></button><button type="button" className="ax-xo-icon-button" aria-label={soundOn?'Turn sound off':'Turn sound on'} aria-pressed={soundOn} onClick={()=>setSoundOn(value=>!value)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 5 5 9H2v6h3l5 4z"/>{soundOn?<path d="M15 8a6 6 0 0 1 0 8m3-11a10 10 0 0 1 0 14"/>:<path d="m16 9 6 6m0-6-6 6"/>}</svg></button><button type="button" className="ax-xo-icon-button ax-xo-close" aria-label="Minimize AX Playroom" onClick={minimizeGame}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button></div></header>

   <main className="ax-xo-experience">
    <section className="ax-xo-editorial" aria-labelledby="ax-xo-title"><div><p className="ax-xo-eyebrow">{launchMode?'The AX Launch Playroom':'The AX Playroom'}</p><h1 id="ax-xo-title"><span>Your</span><span className="ax-xo-headline-last"><em>move.</em><svg className="ax-xo-star" viewBox="0 0 40 40" aria-hidden="true"><path d="M20 1v38M1 20h38M6.5 6.5l27 27M33.5 6.5l-27 27"/></svg></span></h1><p className="ax-xo-description">{launchMode?<>AX goes live 1 October at 4 PM IST.<br/>Beat AX before launch.</>:<>Step out of the scroll.<br/>Take on AX in a little friendly rivalry.</>}</p></div><div className="ax-xo-art" aria-hidden="true"><div className="ax-xo-orbit"/><div className="ax-xo-sculpture-shadow"/><div className="ax-xo-art-o"/><div className="ax-xo-art-x"/><span>Nine squares. All yours.</span></div><div><div className="ax-xo-rewards" aria-label={launchMode?'Playroom rewards: best of three, first to two wins for 10 percent off':'Playroom rewards: win 10 percent off, first draw unlocks one bonus round'}><div><span className="ax-xo-reward-number ax-xo-neon-reward">10<small>%</small></span><p><strong>{launchMode?'Beat AX':'Win the round'}</strong>{launchMode?'Launch-day reward.':'One-use reward.'}</p></div><i/><div><span className="ax-xo-reward-number">{launchMode?'3':'1'}<small>{launchMode?'':'×'}</small></span><p><strong>{launchMode?'Best of three':'First draw'}</strong>{launchMode?'First to 2 wins.':'One Bonus Round.'}</p></div></div><p className="ax-xo-demo-caption ax-xo-live-caption">{launchMode?'First to 2 wins the 10% launch-day reward. Draws award no point.':'Win 10% off. First draw unlocks one Bonus Round. Then the Playroom rests until tomorrow.'}</p></div></section>

    <section className="ax-xo-play-side" aria-label="Play tic-tac-toe with AX"><div className="ax-xo-play-shell"><div className="ax-xo-game-top"><span>You × AX</span><span>{launchMode?'BEST OF 3 · FIRST TO 2':bonusRound?'BONUS ROUND':rounds?'Round '+String(rounds).padStart(2,'0'):'A friendly match'}</span></div><div className="ax-xo-players" data-turn={turn}><span className="ax-xo-turn-track" aria-hidden="true"/><div className={'ax-xo-player'+(turn==='O'?' active':'')}><span className="ax-xo-player-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="7.5"/></svg></span><span>You<small>Always O</small></span><i/></div><div className={'ax-xo-player ax'+(turn==='X'?' active':'')}><span className="ax-xo-player-icon"><svg viewBox="0 0 24 24"><path d="m5 5 14 14M19 5 5 19"/></svg></span><span>AX<small>Always X</small></span><i/></div></div>

     <div className="ax-xo-stage-wrap">
      {phase==='choose'&&<section className="ax-xo-stage ax-xo-choose-stage"><div className="ax-xo-choose-top"><div className="ax-xo-opening-marks" aria-hidden="true"><span className="o"><OMark/></span><em>vs.</em><span><XMark/></span></div><h2>{launchMode?'Best of three.':bonusRound?'Bonus Round.':'You’re O. AX is X.'}</h2><p>{launchMode?'First to 2 wins. Choose who starts.':bonusRound?'One last shot at 10%. Choose who starts.':'Who makes the first move?'}</p></div><div className="ax-xo-choices"><button type="button" className="primary" disabled={starting} onClick={()=>start('O')}><span className="symbol"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="7.5"/></svg></span><span><strong>{starting?'Preparing…':'I’ll go first'}</strong><small>Set the pace. You start with O.</small></span><Arrow/></button><button type="button" disabled={starting} onClick={()=>start('X')}><span className="symbol"><svg viewBox="0 0 24 24"><path d="m6 6 12 12M18 6 6 18"/></svg></span><span><strong>Let AX start</strong><small>AX opens with X. You’re still O.</small></span><Arrow/></button></div></section>}
      ...
      {phase==='result'&&<section className={'ax-xo-stage ax-xo-result-stage outcome-'+(result?.winner||'')+(launchWonMatch?' premium-win':'')}>{result?.winner==='O'&&!launchMode&&<div className="ax-xo-confetti" aria-hidden="true">{Array.from({length:22},(_,index)=><i key={index}/>)}</div>}<div className="ax-xo-result-badge" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M8 3h8v6a4 4 0 0 1-8 0zM8 5H4v2a4 4 0 0 0 4 4m8-6h4v2a4 4 0 0 1-4 4m-4 2v5m-4 3h8m-10 0h12"/></svg></div><p className="ax-xo-result-kicker">{resultKicker}</p><h2>{resultTitle}</h2><p className="ax-xo-result-copy">{resultCopy}</p>
       {result?.winner==='O'&&(!launchMode||launchWonMatch)&&<div className={'ax-xo-ticket '+(settlement.state==='error'?'has-error':'')}><div className="ax-xo-ticket-percent"><b>10</b><small>%</small><span>AX reward</span></div><div className="ax-xo-ticket-info"><strong>{settlement.state==='ready'&&settlement.rewardCode?settlement.rewardCode:settlement.state==='error'?'REWARD PENDING':'CREATING CODE…'}</strong><p>{settlement.state==='error'?settlement.error:(launchMode?'Applicable at launch checkout · 1 October 4 PM IST · launch day only':'Single use · valid 7 days · non-stackable')}</p></div><button type="button" className="ax-xo-copy" disabled={!settlement.rewardCode} onClick={copyCode} aria-label="Copy 10 percent reward code"><svg viewBox="0 0 24 24"><rect x="8" y="8" width="12" height="13" rx="2"/><path d="M16 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h3"/></svg></button></div>}
       {settlement.state==='error'&&<button type="button" className="ax-xo-retry-reward" onClick={retrySettlement}>Retry verification</button>}
       <div className="ax-xo-result-actions">{launchSeriesActive?<button type="button" className="solid" disabled={settlement.state!=='ready'} onClick={()=>resetRound(false)}>Next round <span aria-hidden="true">↗</span></button>:launchWonMatch?<button type="button" className="solid" disabled={!settlement.rewardCode} onClick={copyCode}>Copy 10% code <span aria-hidden="true">↗</span></button>:launchMode&&launchMatchComplete?<button type="button" className="solid" onClick={minimizeGame}>Back to launch <span aria-hidden="true">↗</span></button>:result?.winner==='draw'&&settlement.bonusAvailable?<button type="button" className="solid" disabled={settlement.state!=='ready'} onClick={()=>resetRound(true)}>Play bonus round <span aria-hidden="true">↗</span></button>:result?.winner==='draw'?<button type="button" className="solid" disabled={settlement.state!=='ready'} onClick={minimizeGame}>Back to AX <span aria-hidden="true">↗</span></button>:result?.winner==='O'?<button type="button" className="solid" disabled={!settlement.rewardCode} onClick={copyCode}>Copy 10% code <span aria-hidden="true">↗</span></button>:<button type="button" className="solid" onClick={minimizeGame}>Back to AX <span aria-hidden="true">↗</span></button>}{launchWonMatch?<a className="ax-xo-calendar-action" href="/coming-soon/launch.ics" onClick={()=>trackLaunchEvent('launch_calendar_click',{metadata:{source:'win-popup',provider:'ics'}})}>Mark your calendars <span aria-hidden="true">↗</span></a>:<button type="button" onClick={()=>setPhase('review')}>See final board</button>}</div><p className="ax-xo-result-footnote">{launchMode?(launchWonMatch?'Limited stock. Your verified 10% discount applies at checkout on launch day.':launchMatchComplete?'Best-of-three complete.':'Best of three · first to 2 wins · draws award no point.'):result?.winner==='O'?'One code per verified win. The Playroom returns in 7 days.':result?.winner==='draw'?(settlement.bonusAvailable?'One draw unlocks one Bonus Round only.':'Bonus Round complete. Your next Playroom unlocks tomorrow.'):'No discount, no penalty. Your next round unlocks tomorrow.'}</p></section>}
     </div>

     <div className={'ax-xo-game-bottom'+(thinking?' thinking':'')}><div className="ax-xo-live-status"><span/><span role="status" aria-live="polite">{status}</span>{thinking&&<em><i/><i/><i/></em>}</div><button type="button" onClick={()=>setShowRules(true)}>How to play</button></div></div><div className="ax-xo-session"><span>{launchMode?'Best of three':'This session'}</span><div aria-label="Session scores"><span>You <b>{scores.O}</b></span>{!launchMode&&<span>Draw <b>{scores.draw}</b></span>}<span>AX <b>{scores.X}</b></span></div></div></section>
   </main>

   <footer className="ax-xo-footer"><span>{launchMode?'✣ Beat AX before we go live.':'✣ Good style. Great moves.'}</span><span>{launchMode?'Best of 3 · First to 2 · Win 10%':'Win 10% · One draw bonus · Lose nothing.'}</span><a href={launchMode?'/coming-soon/launch.ics':'/products'} onClick={()=>{if(launchMode)trackLaunchEvent('launch_calendar_click',{metadata:{source:'playroom-footer',provider:'ics'}});minimizeGame();}}>{launchMode?'Mark your calendars':'Explore AX'} <span aria-hidden="true">↗</span></a></footer>

   {showRules&&<div className="ax-xo-rules-backdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target)setShowRules(false);}}><section className="ax-xo-rules" role="dialog" aria-modal="true" aria-labelledby="ax-xo-rules-title"><button type="button" onClick={()=>setShowRules(false)} aria-label="Close how to play"><svg viewBox="0 0 24 24"><path d="m6 6 12 12M18 6 6 18"/></svg></button><p className="ax-xo-eyebrow">A quick breather</p><h2 id="ax-xo-rules-title">{launchMode?'Best of three.':'Three makes a win.'}</h2><ol>{launchMode?<><li>You’re always <strong>O</strong>. AX is always <strong>X</strong>. Choose who starts each round.</li><li>Win a board to score one point. The first side to <strong>2 wins</strong> takes the match.</li><li>A draw awards no point and simply moves the match to the next round. Beat AX in the match to unlock the verified 10% launch-day code.</li></>:<><li>You’re always <strong>O</strong>. AX is always <strong>X</strong>. Choose who makes the opening move.</li><li>Tap an empty square. Get three of your marks in a row, column, or diagonal.</li><li><strong>Win:</strong> get one verified 10% off code. <strong>First draw:</strong> unlock one Bonus Round immediately. If the Bonus Round also draws, the Playroom returns tomorrow. <strong>AX wins:</strong> lose nothing and come back tomorrow.</li></>}</ol><p>{launchMode?'Launch rewards are single-use, activate 1 October at 4:00 PM IST, expire at the end of launch day and cannot be combined with other discounts.':'Win codes are single-use, valid for 7 days and cannot be combined with other discounts. A verified win also pauses the Playroom for 7 days.'}</p><p className="demo ax-xo-live-note">Results are verified by AX before a reward code is issued. No purchase is required to play.</p></section></div>}
