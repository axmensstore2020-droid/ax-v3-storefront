// Technical vectors: one highlighted measuring path at a time, no overlapping labels.
const stroke={fill:'none',stroke:'currentColor',strokeWidth:1.7,strokeLinecap:'round',strokeLinejoin:'round'};
const detail={...stroke,strokeWidth:1,opacity:.32};
...
function Garment({kind}){
 if(tops.includes(kind)){
  ...
  return <g>
   <path d={tank?`M148 70 Q160 95 180 95 Q200 95 212 70 L226 77 Q215 125 234 151 L234 ${hem} Q180 ${hem+9} 126 ${hem} L126 151 Q145 125 134 77 Z`:short?`M148 70 Q180 97 212 70 L237 79 Q253 88 270 103 L303 136 L275 169 L238 146 L238 ${hem} Q180 ${hem+9} 122 ${hem} L122 146 L85 169 L57 136 L90 103 Q107 88 123 79 Z`:`M148 70 Q180 97 212 70 L237 79 Q251 85 261 103 L309 266 L281 279 L239 157 L238 ${hem} Q180 ${hem+9} 122 ${hem} L121 157 L79 279 L51 266 L99 103 Q109 85 123 79 Z`} {...stroke}/>
 ...
 if(bottoms.includes(kind)){
  ...
  return <g><path d={`M119 47 Q180 53 241 47 L250 129 Q249 155 246 179 L235 ${y} L192 ${y} L180 164 L168 ${y} L125 ${y} L114 179 Q111 155 110 129 Z`} {...stroke}/><path d={`M117 64 Q180 70 243 64 M180 53 V135 Q179 153 166 163 M124 ${y-9} H168 M192 ${y-9} H236`} {...detail}/>
...
export function measurementPath(kind,field){
 ...
 if(bottoms.includes(kind)){
  ...
  return {waist:'M119 47 Q180 53 241 47',hip:'M110 129 H250',front_rise:'M180 53 V135 Q181 152 180 164',thigh:'M114 177 H179',knee:'M120 247 H173',inseam:`M180 164 L192 ${y}`,outseam:`M241 47 L250 129 Q249 155 246 179 L235 ${y}`,leg_opening:`M192 ${y} H235`}[field];
 }
...
export default function MeasurementIllustration({category,field='chest',label}){
 const path=measurementPath(category,field);
 return <svg viewBox="0 0 360 375" role="img" aria-label={`${label||category}: ${field.replaceAll('_',' ')} measuring path`}>
  <title>{`${label||category} — ${field.replaceAll('_',' ')}`}</title>
  <Garment kind={category}/>
  {path&&<path key={`${category}-${field}`} d={path} fill="none" stroke="#8a523b" strokeWidth="2.6" strokeDasharray="5 5" strokeLinecap="round"/>}
 </svg>;
}
