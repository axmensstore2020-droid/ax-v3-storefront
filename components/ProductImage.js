...
export default function ProductImage({src,alt,sizes='(max-width:700px) 50vw,25vw',eager=false}) {
 ...
 return <img src={imageUrl(src,720)} srcSet={src.startsWith('https://cdn.shopify.com/') ? [180,320,480,720,1000,1400].map(w => imageUrl(src,w)+' '+w+'w').join(', ') : undefined} sizes={sizes} alt={alt} width="800" height="1066" loading={eager ? 'eager' : 'lazy'} fetchPriority={eager ? 'high' : 'auto'} decoding="async" className="product-image"/>;
}
