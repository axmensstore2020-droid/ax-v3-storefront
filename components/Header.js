...
export default function Header() {
 ...
 return <><header className="header"><div className="header-top"><Link className="brand" href="/" aria-label="AX home"><Brand/></Link><span className="brand-location">COIMBATORE · EST. 2020</span><div className="header-actions"><Link className="header-search" href="/products?search=1" aria-label="Search products"><Icon name="search"/><span>Search</span></Link><WishlistNavLink/><button className="bag-button" aria-label={`Bag, ${count} items`} onClick={() => openCart()}>BAG <span>({count})</span></button><button className="categories-button" onClick={() => setMenu(true)} aria-label="Open categories" aria-haspopup="dialog" aria-expanded={menu} aria-controls="ax-category-menu">CATEGORIES <Icon name="chevron" size={16}/></button></div></div></header>
