...
export function StylistProvider({ children }) {
 ...
 return <Context.Provider value={{openStylist:(mode='style',product=null) => setRequest({mode,product})}}>{children}{request && <Panel request={request} onClose={() => setRequest(null)}/>}</Context.Provider>;
}
