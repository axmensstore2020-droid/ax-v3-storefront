...
export default function AXIsland({ accountUrl, accountEnabled=false }) {
 ...
 {profile && <Dialog title="Your AX" className="stylist-dialog" onClose={() => setProfile(false)}><h3 className="editorial sheet-title">Make yourself at home.</h3><p>{accountEnabled?'Sign in and keep your AX account within the storefront.':'Access your orders through your store account.'}</p>{accountUrl ? <a className="solid-button" href={accountUrl}>{accountEnabled?'OPEN AX ACCOUNT':'OPEN MY ACCOUNT'} <Icon name="arrow"/></a> : <p className="muted">Account sign-in will be available when the store opens.</p>}<p className="small muted">Manage optional measurements and preferences in AX Stylist → My fit & style.</p></Dialog>}</>;
}
