...
export default function WishlistPageClient(){
  const [handles,setHandles]=useState([]),[items,setItems]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState('');
  ...
  return <main id="main-content" className="wishlist-page">
    ...
    {error&&<div className="wishlist-empty" role="alert"><h2 className="editorial">We couldn’t load your saved pieces.</h2><p>{error}</p><button className="underlined-link" type="button" onClick={()=>setHandles(readWishlist())}>TRY AGAIN <Icon name="arrow"/></button></div>}
