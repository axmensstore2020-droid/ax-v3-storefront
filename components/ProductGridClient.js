...
import {styleWorlds,canonicalStyle,flattenNavigation,matchesCategory,matchesStyle,matchesSearch} from '../lib/navigation';
...
export default function ProductGridClient({products,title='New in',initialTerm='',initialType='all',initialStyle='',searchOpen=false,collection=false,home=false}) {
 ...
 return <><div className="collection-title"><div><p className="eyebrow">THE AX COLLECTION</p><Heading className="editorial">{heading}</Heading></div><span className="collection-count" role="status">{filtered.length} {filtered.length===1?'piece':'pieces'}</span></div><div className="collection-toolbar">{!home && <label className="search-box"><Icon name="search"/><span className="sr-only">Search products</span><input ref={input} value={term} onChange={e => setTerm(e.target.value)} placeholder="Search the collection" type="search"/></label>}<button className={`wide-toggle${wide?' active':''}`} aria-pressed={wide} onClick={() => setWide(!wide)}><Icon name="wide"/>WIDE VIEW<span className="toggle-track" aria-hidden="true"><span/></span></button></div>
 {!home && <div className="filter-row compact-filters"><Link href="/collections">Browse collections</Link><label className="category-select"><span className="sr-only">Browse categories</span><select aria-label="Browse categories" value={current} onChange={e=>router.push(e.target.value)}><option value="" disabled>Categories</option>{menu.map(item=><option key={item.key} value={item.href}>{item.label}</option>)}</select><Icon name="chevron" size={14}/></label>{(style || category!=='all') && <button className="clear-filter" onClick={reset}>Clear filters <Icon name="close" size={14}/></button>}</div>}
