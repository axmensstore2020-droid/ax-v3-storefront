...
const openingIntroBootstrap=`(function(){var root=document.documentElement,key='ax_opening_intro_v3',reduced=false,play=false,host='',comingSoon=false;try{host=String(window.location.hostname||'').toLowerCase();comingSoon=(host==='axstore.in'||host==='www.axstore.in')&&Date.now()<1790850600000;if(comingSoon){root.dataset.axComingSoonBoot='1';root.dataset.axIntro='skip';return;}reduced=window.matchMedia('(prefers-reduced-motion: reduce)').matches;play=window.location.pathname==='/'&&!reduced&&!window.sessionStorage.getItem(key);if(play)window.sessionStorage.setItem(key,'1');}catch(e){play=false;}root.dataset.axIntro=play?'play':'skip';if(play)window.setTimeout(function(){if(root.dataset.axIntro!=='skip')root.dataset.axIntro='skip';},1450);})();`;
