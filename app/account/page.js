...
export default async function AccountPage({searchParams}) {
  ...
  return <main id="main-content" className="account-page"><Link className="info-back" href="/">AX / ACCOUNT</Link><header className="info-heading"><p className="eyebrow">YOUR AX</p><h1 className="editorial">Make yourself at home.</h1><p>{customer ? `Welcome back${customer.firstName ? `, ${customer.firstName}` : ''}. Your AX account is connected to this storefront.` : 'Sign in to keep your orders and account access close to the AX experience.'}</p></header>{status && <p className="account-notice" role="status">{statusCopy[status] || statusCopy.error}</p>}{customer ? <section className="account-card" aria-label="Account actions"><p className="eyebrow">ACCOUNT</p><h2 className="editorial">Welcome{customer.firstName ? `, ${customer.firstName}` : ''}.</h2><p className="muted">Your customer account is managed securely by Shopify. We’ll keep building the AX account experience here.</p><div className="account-actions"><Link className="solid-button" href="/products">EXPLORE AX <span aria-hidden="true">→</span></Link><form action="/account/logout" method="post"><button className="underlined-link" type="submit">SIGN OUT <span aria-hidden="true">→</span></button></form></div></section> : <a className="solid-button" href="/account/login?returnTo=%2Faccount">SIGN IN TO AX <span aria-hidden="true">→</span></a>}</main>;
}
