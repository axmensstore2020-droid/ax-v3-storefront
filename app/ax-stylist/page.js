...
// Operational/privacy facts must not be replaced by old Shopify page copy.
export default function StylistInfoPage() {
  ...
  return <InfoPage eyebrow="AX STYLIST" title="A little direction. Your own style.">
    ...
    <ul>
      <li>When you agree and send a message, your message, recent chat context, current fit/profile details and any selected clothing photo are processed by OpenAI. AX product and policy data relevant to your request is also sent.</li>
      <li>AX does not save chat transcripts or photos to its database. Recent text is kept temporarily in this open chat in your browser, including a signed continuation token. Closing or clearing the chat removes that local chat state. Photos are used for the current reply only.</li>
      <li>“Save profile” is optional and has a separate agreement. It saves the measurements and preferences you entered in AX’s private Supabase database, linked to this browser using an anonymous cookie, for up to 30 days. It is not linked to your Shopify login and does not follow you across devices.</li>
      <li>An essential anonymous cookie and short-lived usage counters help limit abuse and cost. A protected hash of your IP may be used if the hosting proxy is configured for it. AX application logs record error IDs, not message text or photos.</li>
      <li>OpenAI requests use <code>store:false</code>. This does not mean zero retention: OpenAI may retain data for abuse monitoring or other documented purposes. Read <a href="https://developers.openai.com/api/docs/guides/your-data" target="_blank" rel="noopener noreferrer">OpenAI’s data controls</a>. Hosting and database providers may retain operational logs or backups under their own settings.</li>
    </ul>
    <h2>Your controls</h2>
    <p>Use “Clear chat” to clear this conversation. Use “Delete saved profile” to remove your stored fit/profile and clear this chat. Deleting the profile does not reset usage limits or delete Shopify accounts, orders or checkout data. Delete your profile before clearing site cookies; otherwise this browser will lose the anonymous link needed to access it.</p>
    ...
    <p>For order help, product measurements or data questions, email <a href="mailto:contact@axstore.in">contact@axstore.in</a>. Shopify remains responsible for your shopping bag, account, checkout and orders.</p>
