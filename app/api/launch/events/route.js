...
const VISITOR_COOKIE='ax_launch_visitor';
...
function json(body,status=200,headers={}){return NextResponse.json(body,{status,headers:{'Cache-Control':'no-store, private','X-Content-Type-Options':'nosniff','Cross-Origin-Resource-Policy':'same-origin',...headers}});}
function visitor(request,secret){
  ...
  return {id,isNew:id!==existing,hash:createHmac('sha256',secret).update('launch:'+id).digest('hex')};
}
...
export async function POST(request){
  ...
  const allowedOrigins=[configuredOrigin,'https://axstore.in','https://www.axstore.in',process.env.NODE_ENV!=='production'?requestOrigin:null].filter(Boolean);
  if(!allowedOrigins.some(origin=>sameOriginRequest(request,origin)))return json({ok:false,error:'Forbidden.'},403);

  const secret=String(process.env.AX_ANALYTICS_SECRET||process.env.AX_STYLIST_SECRET||'');
  if(!databaseConfigured()||secret.length<32)return json({ok:false,error:'Launch analytics is not configured.'},503);

  const burst=reserveAnalyticsBurst(request,{limit:80,windowMs:60_000});
  const headers=burst.setCookie?{'Set-Cookie':`${ANALYTICS_GUARD_COOKIE}=${burst.token}; Path=/api/launch; Max-Age=3600; HttpOnly; SameSite=Strict${process.env.NODE_ENV==='production'?'; Secure':''}`}:{};
  if(!burst.allowed)return json({ok:false,error:'Too many requests.'},429,{...headers,'Retry-After':String(burst.retryAfter)});

  try{
    const body=await readLimitedJson(request,3072);
    assertAllowedKeys(body,['eventName','metadata'],'launch analytics');
    ...
    if(identity.isNew)response.cookies.set(VISITOR_COOKIE,identity.id,{httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'lax',path:'/',maxAge:7*24*60*60});
    return response;
  }catch{
    return json({ok:false,error:'Launch analytics event could not be stored.'},502,headers);
  }
}
