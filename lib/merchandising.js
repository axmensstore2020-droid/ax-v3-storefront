...
export function featuredProductRank(product={}){
 const type=normalize(product.type || product.productType || '');
 const text=' '+productText(product)+' ';

 // AX Featured order: hero garment families first, while the storefront
 // preserves Shopify's original order inside each family.
 if(/\blinen\b/.test(text) && /\bshirts?\b/.test(text))return 0;
 if(type.includes('jacket') || /\bjackets?\b/.test(text))return 1;
 if(type==='sweatpants' || /\bsweatpants?\b/.test(text))return 2;
 if(type==='hoodie' || /\bhoodies?\b/.test(text))return 3;
 if(type==='oversized t shirts')return 4;
 if(type==='long sleeve t shirts')return 5;
 if(type==='sleeveless t shirts')return 6;
 if(type==='jeans')return 7;
 if(type==='trousers')return 8;
 if(type==='shirt')return 9;
 if(type.includes('accessories') || /\bchains?\b/.test(text))return 10;
 return 99;
}
