...
export const productFields=`id handle title description seo{title description} productType vendor tags updatedAt availableForSale featuredImage {url altText width height} priceRange {minVariantPrice {amount currencyCode}} compareAtPriceRange {minVariantPrice {amount currencyCode}} selectedOrFirstAvailableVariant {sku price{amount currencyCode} compareAtPrice{amount currencyCode} weight weightUnit requiresShipping} options{name optionValues{name}}`;
export const productMetafieldsQuery=`query ProductMetafields($handle:String!){product(handle:$handle){id metafields(identifiers:[${AX_CURRENT_PRODUCT_IDENTIFIERS}]) {namespace key value}}}`;
...
export const productInventoryQuery=`query ProductInventory($handle:String!){product(handle:$handle){id variants(first:100){nodes{id quantityAvailable}}}}`;
...
export const productQuery=`query Product($handle:String!){product(handle:$handle){${productFields} media(first:100){nodes{... on Video{id mediaContentType alt previewImage{url} sources{url mimeType height}}}} images(first:8){nodes{url altText width height}} options{name optionValues{name}} variants(first:100){nodes{id title sku availableForSale price{amount currencyCode} compareAtPrice{amount currencyCode} weight weightUnit requiresShipping selectedOptions{name value} image{url altText}}}}}`;
