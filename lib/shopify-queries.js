...
export const productFields=`id handle title description productType vendor tags updatedAt availableForSale featuredImage {url altText width height} priceRange {minVariantPrice {amount currencyCode}} selectedOrFirstAvailableVariant {sku} metafields(identifiers:[${AX_PRODUCT_IDENTIFIERS}]) {namespace key value}`;
...
export const collectionsQuery=`query Collections($first:Int!){collections(first:$first,sortKey:UPDATED_AT,reverse:true){nodes{id handle title description updatedAt products(first:1){nodes{id}}}}}`;
export const collectionQuery=`query Collection($handle:String!){collection(handle:$handle){id handle title description updatedAt products(first:100,sortKey:COLLECTION_DEFAULT){nodes{${productFields}}}}}`;
export const productQuery=`query Product($handle:String!){product(handle:$handle){${productFields} images(first:8){nodes{url altText width height}} options{name optionValues{name}} variants(first:100){nodes{id title sku availableForSale price{amount currencyCode} selectedOptions{name value} image{url altText}}}}}`;
export const cartFields=`id checkoutUrl totalQuantity cost{subtotalAmount{amount currencyCode} totalAmount{amount currencyCode}} lines(first:100){nodes{id quantity cost{totalAmount{amount currencyCode}} merchandise{... on ProductVariant{id title selectedOptions{name value} image{url altText} product{title handle featuredImage{url altText}} price{amount currencyCode}}}}}`;
