export const PARTIAL_COD_MIN_ADVANCE_INR=100;
export const PARTIAL_COD_RATE=0.10;
export const PARTIAL_COD_ROUNDING_INR=10;
...
export function partialCodAdvance(orderTotal){
 ...
 const tenPercentPaise=Math.round(totalPaise*PARTIAL_COD_RATE);
 const stepPaise=PARTIAL_COD_ROUNDING_INR*100;
 const roundedTenPercentPaise=Math.round(tenPercentPaise/stepPaise)*stepPaise;
 const minimumPaise=PARTIAL_COD_MIN_ADVANCE_INR*100;
 const advancePaise=Math.min(totalPaise,Math.max(minimumPaise,roundedTenPercentPaise));
