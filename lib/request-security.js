...
export function reserveCartBurst(request,{limit=40,windowMs=60_000,ipHeader=process.env.SHOPIFY_BUYER_IP_HEADER,now=Date.now()}={}) {
  const safeLimit=Math.min(120,Math.max(10,Number(limit)||40));
  ...
  let token=cookieValue(request,CART_GUARD_COOKIE),setCookie=false,key;
  if(rawIp && isIP(rawIp)) key='ip:'+hashKey(rawIp);
  else {
