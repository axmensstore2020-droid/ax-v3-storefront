...
export {readLimitedJson} from './request-body.js';
...
export const RESTOCK_GUARD_COOKIE='ax_restock_guard';
export const ERROR_GUARD_COOKIE='ax_error_guard';
export const CATALOG_GUARD_COOKIE='ax_catalog_guard';
export const PLAYROOM_GUARD_COOKIE='ax_playroom_guard';
...
export function reserveRestockBurst(request,{limit=10,windowMs=300_000,ipHeader=process.env.SHOPIFY_BUYER_IP_HEADER,now=Date.now()}={}) {
  return reserveAnonymousBurst(request,{cookieName:RESTOCK_GUARD_COOKIE,limit,windowMs,ipHeader,now});
}

export function reserveErrorBurst(request,{limit=12,windowMs=60_000,ipHeader=process.env.SHOPIFY_BUYER_IP_HEADER,now=Date.now()}={}) {
  return reserveAnonymousBurst(request,{cookieName:ERROR_GUARD_COOKIE,limit,windowMs,ipHeader,now});
}

export function reserveCatalogBurst(request,{limit=80,windowMs=60_000,ipHeader=process.env.SHOPIFY_BUYER_IP_HEADER,now=Date.now()}={}) {
  return reserveAnonymousBurst(request,{cookieName:CATALOG_GUARD_COOKIE,limit,windowMs,ipHeader,now});
}

export function reservePlayroomBurst(request,{limit=20,windowMs=60_000,ipHeader=process.env.SHOPIFY_BUYER_IP_HEADER,now=Date.now()}={}) {
  return reserveAnonymousBurst(request,{cookieName:PLAYROOM_GUARD_COOKIE,limit,windowMs,ipHeader,now});
}

export function reserveAccountBurst(request,{limit=30,windowMs=60_000,now=Date.now()}={}) {
  const token=cookieValue(request,'ax_customer_account_session');
  if(!token) return {allowed:false,retryAfter:60};
  const safeLimit=Math.min(120,Math.max(5,Number(limit)||30));
  const safeWindow=Math.min(5*60_000,Math.max(10_000,Number(windowMs)||60_000));
  const key='account:'+hashKey(token);
  prune(now);
  if(!buckets.has(key) && buckets.size>=MAX_BUCKETS) return {allowed:false,retryAfter:60};
  const bucket=buckets.get(key);
  if(!bucket || bucket.resetAt<=now) {
    buckets.set(key,{count:1,resetAt:now+safeWindow});
    return {allowed:true,retryAfter:0};
  }
  if(bucket.count>=safeLimit) return {allowed:false,retryAfter:Math.max(1,Math.ceil((bucket.resetAt-now)/1000))};
  bucket.count+=1;
  return {allowed:true,retryAfter:0};
}
