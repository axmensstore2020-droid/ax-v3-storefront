import 'server-only';
import {productVideos} from './product-media.js';
// SHOPIFY STOREFRONT BOUNDARY
// Catalog, price, variants, inventory and normal checkout facts come from Shopify.
// Fix bad merchant data in Shopify rather than hard-coding a contradictory value here.
...
import {productsQuery,productsByHandlesQuery,homepageProductsQuery,collectionsQuery,collectionQuery,productQuery,productQueryWithoutInventory,legacyProductMetafieldsQuery,merchantProductsQuery,merchandisingProductsQuery,purchaseProductsByHandlesQuery} from './shopify-queries.js';
...
export async function storefront(query,variables={}, {revalidate=60,buyerIp}={}) {
 ...
 if(json.errors?.length) {const error=new Error('The store could not complete this request.');error.code='SHOPIFY_GRAPHQL_ERROR';throw error;}
 return json.data;
}
function normalizeProduct(p){
 ...
 return normalizeProductData({id:p.id,handle:p.handle,title:p.title,description:p.description,seo:p.seo || null,vendor:p.vendor || '',type:p.productType || 'Menswear',tags:p.tags || [],createdAt:p.createdAt || '',updatedAt:p.updatedAt || '',metafields:p.metafields || [],sku:firstVariant?.sku || '',variantSku:firstVariant?.sku || '',availableForSale:p.availableForSale,image:p.featuredImage?.url || '',imageAlt:p.featuredImage?.altText || p.title,images:p.images?.nodes || [],videos:productVideos(p.media?.nodes || []),variantImage:firstVariant?.image || null,price,currency:firstVariant?.price?.currencyCode || p.priceRange?.minVariantPrice?.currencyCode || 'INR',compareAtPrice,options:productOptions({...p,variants:p.variants?.nodes || []}),variants:p.variants?.nodes || [],demo:false});
}
...
export async function getProductsByHandles(handles=[]){
 const safe=[...new Set((Array.isArray(handles)?handles:[]).map(value=>String(value||'').trim().toLowerCase()).filter(value=>/^[a-z0-9][a-z0-9-]{0,179}$/.test(value)))].slice(0,40);
 if(!safe.length)return [];
 const data=await storefront(productsByHandlesQuery,{first:safe.length,query:safe.map(handle=>'handle:'+handle).join(' OR ')});
 if(!data)return demoProducts().filter(product=>safe.includes(product.handle));
 const byHandle=new Map(data.products.nodes.filter(isStoreProduct).map(normalizeProduct).map(product=>[product.handle,product]));
 return safe.map(handle=>byHandle.get(handle)).filter(Boolean);
}

export const getMerchandisingProducts=cache(async(first=250) => {
 const data=await storefront(merchandisingProductsQuery,{first});
 return data?data.products.nodes.filter(isStoreProduct).map(normalizeProduct):demoProducts().slice(0,first);
});

export async function getCompleteLookProducts(handles=[]){
 const safe=[...new Set((Array.isArray(handles)?handles:[]).map(value=>String(value||'').trim().toLowerCase()).filter(value=>/^[a-z0-9][a-z0-9-]{0,179}$/.test(value)))].slice(0,10);
 if(!safe.length)return [];
 let batched=[];
 try{
  const data=await storefront(purchaseProductsByHandlesQuery,{first:safe.length,query:safe.map(handle=>'handle:'+handle).join(' OR ')});
  if(!data)return demoProducts().filter(product=>safe.includes(product.handle));
  const byHandle=new Map(data.products.nodes.filter(isStoreProduct).map(normalizeProduct).map(product=>[product.handle,product]));
  batched=safe.map(handle=>byHandle.get(handle)).filter(Boolean);
 }catch{}
 if(batched.length===safe.length)return batched;
 const recovered=new Map(batched.map(product=>[product.handle,product]));
 const missing=safe.filter(handle=>!recovered.has(handle));
 const products=await Promise.all(missing.map(async handle=>{try{return await getProduct(handle);}catch{return null;}}));
 for(const product of products)if(product)recovered.set(product.handle,product);
 return safe.map(handle=>recovered.get(handle)).filter(Boolean);
}
...
export const getMerchantProducts=cache(async(first=100) => {
 const data=await storefront(merchantProductsQuery,{first},{revalidate:900});
 return data?data.products.nodes.filter(isStoreProduct).map(normalizeProduct):demoProducts();
});
...
export const getProduct=cache(async handle => {
 let data;
 try {
  data=await storefront(productQuery,{handle});
 } catch(error) {
  if(error?.code!=='SHOPIFY_GRAPHQL_ERROR') throw error;
  data=await storefront(productQueryWithoutInventory,{handle});
 }
 ...
 let metafields=p.metafields || [];
 if(!metafields.length) {
  try {
   const legacy=await storefront(legacyProductMetafieldsQuery,{handle});
   metafields=legacy?.product?.metafields || [];
  } catch {}
 }
 const enriched={...p,metafields};
 return normalizeProductData({...normalizeProduct(enriched),images:p.images?.nodes || [],options:productOptions({...p,variants:p.variants?.nodes || []}),variants:p.variants?.nodes || []});
});
