...
import {productsQuery,productsByHandlesQuery,homepageProductsQuery,collectionsQuery,collectionQuery,productQuery,productMetafieldsQuery,legacyProductMetafieldsQuery,productInventoryQuery,merchantProductsQuery} from './shopify-queries.js';
...
export const getProduct=cache(async handle => {
 const [coreResult,metaResult,inventoryResult]=await Promise.allSettled([
  storefront(productQuery,{handle}),
  storefront(productMetafieldsQuery,{handle}),
  storefront(productInventoryQuery,{handle})
 ]);
 ...
 let metafields=metaResult.status==='fulfilled' ? (metaResult.value?.product?.metafields || []) : [];
 ...
 const quantityById=new Map(
  inventoryResult.status==='fulfilled'
    ? (inventoryResult.value?.product?.variants?.nodes || []).map(variant=>[variant.id,variant.quantityAvailable])
    : []
 );
 const variants=(p.variants?.nodes || []).map(variant=>quantityById.has(variant.id)?{...variant,quantityAvailable:quantityById.get(variant.id)}:variant);
 const enriched={...p,metafields,variants:{...p.variants,nodes:variants}};
 return normalizeProductData({...normalizeProduct(enriched),images:p.images.nodes,options:productOptions({...p,variants}),variants});
});
