...
export const getProduct=cache(async handle => {
 ...
 return normalizeProductData({...normalizeProduct(p),images:p.images.nodes,options:p.options.map(option => ({name:option.name,values:option.optionValues.map(value => value.name)})),variants:p.variants.nodes});
});
