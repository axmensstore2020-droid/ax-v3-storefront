...
import {productsQuery,collectionsQuery,collectionQuery,productQuery} from './shopify-queries.js';
...
function normalizeProduct(p){
 ...
 return normalizeProductData({id:p.id,handle:p.handle,title:p.title,description:p.description,vendor:p.vendor || '',type:p.productType || 'Menswear',tags:p.tags || [],updatedAt:p.updatedAt || '',metafields:p.metafields || [],sku:firstVariant?.sku || '',variantSku:firstVariant?.sku || '',availableForSale:p.availableForSale,image:p.featuredImage?.url || '',imageAlt:p.featuredImage?.altText || p.title,price:Number(p.priceRange?.minVariantPrice?.amount || 0),currency:p.priceRange?.minVariantPrice?.currencyCode || 'INR',demo:false});
}
function demoProduct(p) {
 ...
 return {...normalized,options:[{name:'Size',values:sizes}],variants:sizes.map(size => ({id:`demo-${normalized.id}-${size}`,title:size,sku:`${normalized.productNumber}-${size}`,availableForSale:true,price:{amount:String(normalized.price),currencyCode:normalized.currency},selectedOptions:[{name:'Size',value:size}]}))};
}
...
export const getCollections=cache(async(first=100) => {
 ...
 if(data?.collections?.nodes) return data.collections.nodes;
 const seen=new Set();
 return [...navigation,...styleWorlds,...seasonalCollections].filter(item=>item?.href?.startsWith('/collections/')).filter(item=>!seen.has(item.key) && seen.add(item.key)).map(item=>({id:item.key,handle:item.key,title:item.label,description:'',updatedAt:''}));
});
export const getCollection=cache(async handle => {
 ...
 if(item) return {id:canonical,handle,title:item.label,description:'',updatedAt:'',products:(await getProducts()).filter(p=>style?matchesStyle(p,canonical):matchesCategory(p,canonical))};
 return null;
});
