...
export function assertAllowedFormKeys(form,allowed){
  const permitted=new Set(allowed),seen=new Set();
  for(const key of form.keys()){
    if(!permitted.has(key)) throw new Error('Unsupported form field.');
    if(seen.has(key)) throw new Error('Duplicate form field.');
    seen.add(key);
  }
  return form;
}
