...
import {replayPlayroomRound} from './playroom-game.js';
...
export function unsealPlayroomCooldown(token,secret,now=Date.now()){
  return unsealEncryptedToken(token,secret,{
    ...
    validate:value=>value?.kind==='playroom-cooldown'&&['win','loss'].includes(value.outcome)&&Number(value.exp)>now
  });
}

export function playroomStatusFromToken(token,{env=process.env,now=Date.now()}={}){
  const config=playroomConfig(env);
  if(!config.ready)return {available:false,eligible:false,nextEligibleAt:null,lastOutcome:null};
  const cooldown=unsealPlayroomCooldown(token,config.secret,now);
  return cooldown?{available:true,eligible:false,nextEligibleAt:new Date(cooldown.exp).toISOString(),lastOutcome:cooldown.outcome}:{available:true,eligible:true,nextEligibleAt:null,lastOutcome:null};
}

export function createPlayroomRound(first,{env=process.env,now=Date.now()}={}){
  const config=playroomConfig(env);
  ...
  const token=sealEncryptedToken({kind:'playroom-game',first,seed,nonce,iat:now,exp},config.secret,'pxg1');
  return {seed,token,expiresAt:new Date(exp).toISOString()};
}

function unsealRound(token,secret,now){
  return unsealEncryptedToken(token,secret,{
    prefix:'pxg1',minLength:30,maxLength:5000,
    validate:value=>value?.kind==='playroom-game'&&(value.first==='O'||value.first==='X')&&typeof value.seed==='string'&&typeof value.nonce==='string'&&Number(value.exp)>now
  });
}
...
export async function settlePlayroomRound(token,moves,{env=process.env,fetchImpl=fetch,now=Date.now()}={}){
  ...
  if(result.winner==='draw')return {outcome:'draw',rewardCode:null,rewardEndsAt:null,cooldownToken:null,nextEligibleAt:null};
  if(result.winner==='X'){
    ...
    return {outcome:'loss',rewardCode:null,rewardEndsAt:null,cooldownToken,nextEligibleAt:new Date(exp).toISOString()};
  }
  ...
  return {outcome:'win',rewardCode:reward.code,rewardEndsAt:reward.endsAt,cooldownToken,nextEligibleAt:new Date(exp).toISOString()};
}
