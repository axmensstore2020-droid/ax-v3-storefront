...
export async function settlePlayroomRound(token,moves,{env=process.env,fetchImpl=fetch,now=Date.now()}={}){
  ...
  if(result.winner==='draw'){
    ...
    if(session.bonus)return {outcome:'draw',bonusAvailable:false,rewardCode:null,rewardEndsAt:null,bonusToken:null,cooldownToken,nextEligibleAt:new Date(exp).toISOString()};
    const bonusToken=sealEncryptedToken({kind:'playroom-bonus',available:true,iat:now,exp},config.secret,'pxb1');
    return {outcome:'draw',bonusAvailable:true,rewardCode:null,rewardEndsAt:null,bonusToken,cooldownToken,nextEligibleAt:null};
  }
  if(result.winner==='X'){
    const exp=nextPlayroomDay(now);
    const cooldownToken=sealEncryptedToken({kind:'playroom-cooldown',outcome:'loss',iat:now,exp},config.secret,'pxc1');
    return {outcome:'loss',bonusAvailable:false,rewardCode:null,rewardEndsAt:null,bonusToken:null,cooldownToken,nextEligibleAt:new Date(exp).toISOString()};
  }
  ...
  return {outcome:'win',bonusAvailable:false,rewardCode:reward.code,rewardEndsAt:reward.endsAt,bonusToken:null,cooldownToken,nextEligibleAt:new Date(exp).toISOString()};
}
