...
export const PLAYROOM_SERIES_COOKIE='ax_playroom_series';
export const PLAYROOM_GAME_MAX_AGE=20*60;
export const PLAYROOM_SERIES_MAX_AGE=24*60*60;
export const PLAYROOM_WIN_COOLDOWN_MS=7*24*60*60*1000;
export const AX_LAUNCH_START_MS=Date.parse('2026-10-01T10:30:00.000Z');
export const AX_LAUNCH_END_MS=Date.parse('2026-10-01T18:29:59.000Z');
...
function unsealPlayroomSeries(token,secret,now=Date.now()){
  return unsealEncryptedToken(token,secret,{
    prefix:'pxs1',minLength:30,maxLength:4000,
    validate:value=>value?.kind==='playroom-series'&&Number.isInteger(value.you)&&value.you>=0&&value.you<2&&Number.isInteger(value.ax)&&value.ax>=0&&value.ax<2&&Number.isInteger(value.draws)&&value.draws>=0&&Number.isInteger(value.rounds)&&value.rounds>=0&&Number(value.exp)>now
  });
}

export function playroomSeriesFromToken(token,{env=process.env,now=Date.now()}={}){
  const config=playroomConfig(env);
  if(!config.ready)return {you:0,ax:0,draws:0,rounds:0};
  const series=unsealPlayroomSeries(token,config.secret,now);
  return series?{you:series.you,ax:series.ax,draws:series.draws,rounds:series.rounds}:{you:0,ax:0,draws:0,rounds:0};
}

export function createPlayroomRound(first,{bonus=false,launch=false,env=process.env,now=Date.now()}={}){
  const config=playroomConfig(env);
  ...
  const token=sealEncryptedToken({kind:'playroom-game',first,seed,nonce,bonus:Boolean(bonus),launch:Boolean(launch),iat:now,exp},config.secret,'pxg1');
  return {seed,token,bonus:Boolean(bonus),launch:Boolean(launch),expiresAt:new Date(exp).toISOString()};
}

function unsealRound(token,secret,now){
  return unsealEncryptedToken(token,secret,{
    prefix:'pxg1',minLength:30,maxLength:5000,
    validate:value=>value?.kind==='playroom-game'&&(value.first==='O'||value.first==='X')&&typeof value.seed==='string'&&typeof value.nonce==='string'&&typeof value.bonus==='boolean'&&(typeof value.launch==='boolean'||value.launch===undefined)&&Number(value.exp)>now
  });
}
...
export function getPlayroomAxMove(token,moves,{env=process.env,now=Date.now()}={}){
  ...
  return nextPlayroomAxMove({first:session.first,moves,seed:session.seed,friendly:Boolean(session.launch)});
}
...
export async function createShopifyPlayroomDiscount(code,{launch=false,env=process.env,fetchImpl=fetch,now=Date.now()}={}){
  const config=playroomConfig(env);
  if(!config.ready)throw new Error('AX Playroom rewards are not configured yet.');
  if(launch&&now>=AX_LAUNCH_END_MS)throw new Error('The launch-day reward window has ended.');
  const startsAt=new Date(launch?Math.max(now,AX_LAUNCH_START_MS):now).toISOString();
  const endsAt=new Date(launch?AX_LAUNCH_END_MS:now+PLAYROOM_WIN_COOLDOWN_MS).toISOString();
  const input={
    title:`AX Playroom · ${launch?'Launch day · ':''}10% win · ${code.slice(-10)}`,
    code,
    context:{all:'ALL'},
    customerGets:{items:{all:true},value:{percentage:0.1}},
    startsAt,endsAt,usageLimit:1,appliesOncePerCustomer:true,
    combinesWith:{orderDiscounts:false,productDiscounts:false,shippingDiscounts:false},
    tags:launch?['ax-playroom','10-percent','launch-day']:['ax-playroom','10-percent']
  };
...
export async function settlePlayroomRound(token,moves,{seriesToken='',env=process.env,fetchImpl=fetch,now=Date.now()}={}){
  const config=playroomConfig(env);
  if(!config.ready)throw new Error('AX Playroom rewards are not configured yet.');
  const session=unsealRound(token,config.secret,now);
  if(!session)throw new Error('This Playroom round expired. Start a new round.');
  const {result}=replayPlayroomRound({first:session.first,moves,seed:session.seed,friendly:Boolean(session.launch)});

  if(session.launch){
    const previous=unsealPlayroomSeries(seriesToken,config.secret,now)||{you:0,ax:0,draws:0,rounds:0};
    const next={
      you:previous.you+(result.winner==='O'?1:0),
      ax:previous.ax+(result.winner==='X'?1:0),
      draws:previous.draws+(result.winner==='draw'?1:0),
      rounds:previous.rounds+1
    };
    const roundOutcome=result.winner==='O'?'win':result.winner==='X'?'loss':'draw';
    if(next.you>=2){
      const code=`AXPLAY10-${session.nonce.slice(0,10).toUpperCase()}`;
      const reward=await createShopifyPlayroomDiscount(code,{launch:true,env,fetchImpl,now});
      ...
      return {outcome:roundOutcome,matchComplete:true,matchOutcome:'win',series:next,seriesToken:null,bonusAvailable:false,rewardCode:reward.code,rewardEndsAt:reward.endsAt,bonusToken:null,cooldownToken,cooldownEndsAt:new Date(exp).toISOString(),nextEligibleAt:new Date(exp).toISOString()};
    }
    if(next.ax>=2){
      ...
      return {outcome:roundOutcome,matchComplete:true,matchOutcome:'loss',series:next,seriesToken:null,bonusAvailable:false,rewardCode:null,rewardEndsAt:null,bonusToken:null,cooldownToken,cooldownEndsAt:new Date(exp).toISOString(),nextEligibleAt:new Date(exp).toISOString()};
    }
    const exp=Math.min(now+PLAYROOM_SERIES_MAX_AGE*1000,AX_LAUNCH_END_MS);
    const nextSeriesToken=sealEncryptedToken({kind:'playroom-series',...next,iat:now,exp},config.secret,'pxs1');
    return {outcome:roundOutcome,matchComplete:false,matchOutcome:null,series:next,seriesToken:nextSeriesToken,seriesEndsAt:new Date(exp).toISOString(),bonusAvailable:false,rewardCode:null,rewardEndsAt:null,bonusToken:null,cooldownToken:null,cooldownEndsAt:null,nextEligibleAt:null};
  }
  ...
  const code=`AXPLAY10-${session.nonce.slice(0,10).toUpperCase()}`;
  const reward=await createShopifyPlayroomDiscount(code,{launch:Boolean(session.launch),env,fetchImpl,now});
  const exp=now+PLAYROOM_WIN_COOLDOWN_MS;
  const cooldownToken=sealEncryptedToken({kind:'playroom-cooldown',outcome:'win',iat:now,exp},config.secret,'pxc1');
