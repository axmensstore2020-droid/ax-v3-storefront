...
function sharedProductFields(product,{includeColor=true}={}){
 const images=imageUrls(product),properties=productProperties(product),rating=Number(product?.reviewRating||0),reviewCount=Number(product?.reviewCount||0);
 return {
  name:product.title,
  ...
  ...(rating>=1&&rating<=5&&Number.isInteger(reviewCount)&&reviewCount>0?{aggregateRating:{'@type':'AggregateRating',ratingValue:String(rating),reviewCount:String(reviewCount),bestRating:'5',worstRating:'1'}}:{})
 };
}
