...
export function validateCartInput(body) {
 ...
 const allowed=new Set(['action','cartId','merchandiseId','lineId','quantity','lines']);
 if(Object.keys(body).some(key=>!allowed.has(key))) return 'Unsupported bag field.';
 const {action,cartId,merchandiseId,lineId,quantity=1,lines}=body;
 if(!['get','create','createMany','add','addMany','update','remove'].includes(action)) return 'Unknown cart action.';
 if(!['create','createMany'].includes(action) && (typeof cartId!=='string' || !cartId.startsWith('gid://shopify/Cart/') || cartId.length>1024)) return 'Invalid bag.';
 ...
 if(['createMany','addMany'].includes(action)) {
  ...
  for(const item of lines) if(!item || typeof item!=='object' || Array.isArray(item) || Object.keys(item).some(key=>!['merchandiseId','quantity'].includes(key)) || typeof item.merchandiseId!=='string' || !/^gid:\/\/shopify\/ProductVariant\/\d+$/.test(item.merchandiseId) || !Number.isInteger(item.quantity) || item.quantity<1 || item.quantity>99) return 'Choose valid product options.';
 }
