...
// Merchandiser-authored BODY ranges, never an LLM's height/weight guess.
export function recommendFit(product, profile = {}, selectedOptions = {}) {
  ...
  if (!matches.length) return {status:'outside_chart',message:'Your measurements fall outside the approved ranges for this piece. I cannot confidently recommend a size; please ask AX.',recommendedSize:null,selectedSize:null,selectedFit:null};
  if (matches.length > 1) return {status:'between_sizes',message:`Your measurements overlap sizes ${matches.map(([size]) => size).join(' and ')}. Compare the garment chart with a piece you like, or ask AX to confirm.`,recommendedSize:null,selectedSize:null,selectedFit:null};
  ...
  return {status:available ? 'recommended' : 'recommended_unavailable',recommendedSize,selectedSize,selectedFit,
    message:`${recommendedSize} matches AX’s approved body-size guide for this cut.${available ? '' : ' It is currently unavailable with the selected options.'}${selectedSize && selectedFit ? ` Your selected ${selectedSize} is estimated to feel ${selectedFit}, based on AX’s garment measurements and ease guide.` : ''} This is guidance, not a fit guarantee.`};
}
