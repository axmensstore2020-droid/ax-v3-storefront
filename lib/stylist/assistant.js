...
export async function runStylist({message,image='',history=[],summary='',profile={},productHandle='',selectedOptions={},safetyId,ai,catalog,config=stylistConfig(),reserveAdvanced=async()=>false,onRoute=()=>{}}) {
  ...
  while(modelCalls<config.maxModelCalls) {
    ...
    try {
      modelCalls++;
      const firstToolChoice=route.intent==='catalog' && !productHandle
        ? {type:'function',name:'search_products'}
        : ['catalog','store_help','fit'].includes(route.intent) ? 'required' : 'auto';
      response=await ai.respond({input,tools:TOOLS,parallel_tool_calls:false,tool_choice:forceFinal?'none':modelCalls===1?firstToolChoice:'auto',
    ...
    if(route.intent==='fit' && /recommend|my size|best size|which size|what size.*(?:me|wear)|my measurements/i.test(message) && !fits.size) text=productHandle
      ? 'Add your measurements in My fit & style and I’ll check this piece against AX’s size guide. I can still help with how the cut, fabric and styling will wear.'
      : 'Tell me the piece you’re considering, or what you’re shopping for—like a formal shirt or relaxed linen shirt. I’ll find AX options first, then check fit once you choose one.';
