...
export function databaseConfigured(env = process.env) {
  return /^https:\/\/[a-z0-9-]+\.supabase\.co$/.test(env.SUPABASE_URL || '') && Boolean(env.SUPABASE_SERVICE_ROLE_KEY);
}
export function createDatabase(env = process.env,fetcher = fetch) {
  async function call(path,method='GET',body) {
    ...
    const response = await fetcher(env.SUPABASE_URL+'/rest/v1/'+path,{
      method,headers:{apikey:env.SUPABASE_SERVICE_ROLE_KEY,Authorization:'Bearer '+env.SUPABASE_SERVICE_ROLE_KEY,'Content-Type':'application/json',Prefer:'return=representation,resolution=merge-duplicates'},
