...
const fashion=/\b(?:wear|outfit|styl(?:e|ing)|fashion|shirt|jeans|pants|trousers|cargos?|jacket|dress|fit|colou?r|fabric|sizing|garment|look|wardrobe|kurta|sherwani|achkan|bandhgala|jodhpuri|nehru|mundu|veshti|dhoti|mojari|jutti|blazer|suit|streetwear)\b/i;
...
export function routeAXStylistRequest(context,config=stylistConfig()) {
  ...
  else if (/\b(?:outfit|wear|match|pair|style|styling|look|goes with|colou?r.*(?:work|suit)|trending|kurta|sherwani|achkan|bandhgala|jodhpuri|nehru|mundu|veshti|dhoti|mojari|jutti|streetwear)\b/i.test(message)) intent='styling';
  ...
  if (['styling','fit','comparison','photo_styling'].includes(intent)) {
    const constraints=[/\b(?:under|budget|₹|rs\.?|inr)\b/i,/\b(?:wedding|reception|sangeet|mehendi|haldi|nikah|engagement|date|office|party|occasion|interview|vacation|festival|onam|diwali|eid)\b/i,/\b(?:prefer|avoid|dislike|no prints|without|modest)\b/i,/\b(?:hot|humid|cold|winter|summer|weather)\b/i,/\b(?:relaxed|oversized|slim|regular|korean|old money|streetwear|formal)\b/i].filter(pattern=>pattern.test(message)).length;
