...
// `ax` metafields below hold the merchandising information that helps a
...
const METAFIELD_ALIASES = {
  ...
  measurements: ['measurements', 'measurement', 'size_guide'],
...
function metafieldMap(product) {
  return (product?.metafields || []).reduce((map, field) => {
    ...
    if (field.namespace && field.namespace !== 'ax') return map;
...
export function recommendedSizeFromMeasurements(measurements, input, dimension = 'chest') {
  ...
  return rows.find(row => row.value >= numericInput)?.size || rows.at(-1)?.size || '';
}
