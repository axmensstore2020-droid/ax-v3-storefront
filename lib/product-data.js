...
const METAFIELD_ALIASES = {
  ...
  color: ['color', 'colour'],
  colorGroup: ['color_group', 'colour_group'],
  style: ['style'],
  ...
  ugcImages: ['ugc_images', 'customer_images']
};
...
export function normalizeProductData(product = {}) {
  ...
  const colorGroup = pickField(metafields, tags, description, 'colorGroup') ?? product.colorGroup;
  ...
  const reviewRating = Number(pickField(metafields, tags, description, 'reviewRating') ?? product.reviewRating ?? 0);
  const reviewCount = Number(pickField(metafields, tags, description, 'reviewCount') ?? product.reviewCount ?? 0);
  const rawUgc = parseJson(pickField(metafields, tags, description, 'ugcImages') ?? product.ugcImages ?? []);
  const ugcImages = (Array.isArray(rawUgc)?rawUgc:[]).map(item=>typeof item==='string'?item:item?.url).filter(url=>{try{return new URL(url).protocol==='https:';}catch{return false;}}).slice(0,6);
  ...
  return {
    ...
    reviewRating: Number.isFinite(reviewRating)&&reviewRating>=1&&reviewRating<=5?Math.round(reviewRating*10)/10:0,
    reviewCount: Number.isInteger(reviewCount)&&reviewCount>0?Math.min(reviewCount,1000000):0,
    ugcImages,
