import 'server-only';
// PARTIAL COD MONEY/ORDER BOUNDARY
// Flow: Delhivery quote -> Razorpay advance -> verified payment -> Shopify partially-paid
// order -> Delhivery COD shipment. Preserve idempotency/retry protections: one Razorpay
// payment must never create duplicate Shopify orders or shipments.
...
export async function confirmPartialCod({sessionToken,razorpayOrderId,razorpayPaymentId,razorpaySignature},{env=process.env}={}) {
  ...
  return {
    ...
    codBalance:orderMoney(order,'totalOutstandingSet'),
