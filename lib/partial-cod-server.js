...
const validCartId=value=>typeof value==='string' && value.startsWith('gid://shopify/Cart/') && value.length>24 && value.length<=1024 && !/\\s/.test(value) && !value.includes('\\0');
...
export function partialCodConfig(env=process.env) {
  ...
  const adminToken=text(env.SHOPIFY_ADMIN_ACCESS_TOKEN,320),sessionSecret=String(env.AX_PARTIAL_COD_SECRET || '');
  ...
  return {
    enabled,domain,keyId,keySecret,adminToken,sessionSecret,apiVersion,delhiveryReady,
    testMode:keyId.startsWith('rzp_test_'),
    ready:Boolean(enabled && /^[a-z0-9-]+\.myshopify\.com$/.test(domain) && keyId && keySecret && adminToken && sessionSecret.length>=32 && delhiveryReady && shopifyConfigured())
  };
}
...
export async function quotePartialCod(cartId,pincode,{env=process.env}={}) {
  ...
  if(!estimate.configured) throw new Error('COD delivery is not configured.');
  if(estimate.error && !estimate.serviceable) throw new Error(estimate.error);
  if(!estimate.codServiceable || !estimate.serviceable) throw new Error('Cash on Delivery is not available for this pincode.');
  if(estimate.needsWeight) throw new Error('One or more items are missing a shipping weight.');
  if(!estimate.rates?.length) throw new Error(estimate.error || 'Delhivery did not return a COD delivery option.');
...
export function sealPartialCodSession(value,secret) {
  ...
  return `pc1.${iv.toString('base64url')}.${cipher.getAuthTag().toString('base64url')}.${encrypted.toString('base64url')}`;
}

export function unsealPartialCodSession(token,secret) {
  if(typeof token!=='string' || token.length<30 || token.length>24000 || !secret || secret.length<32) return null;
  const parts=token.split('.');
  if(parts.length!==4 || parts[0]!=='pc1') return null;
  try {
    ...
    return value?.kind==='partial-cod' && Number(value.exp)>Date.now() ? value : null;
  } catch {return null;}
}
...
async function adminGraphql(query,variables,config,fetchImpl=fetch) {
  const response=await fetchImpl(`https://${config.domain}/admin/api/${config.apiVersion}/graphql.json`,{
    method:'POST',headers:{Accept:'application/json','Content-Type':'application/json','X-Shopify-Access-Token':config.adminToken},
...
async function createShopifyPartialCodOrder(session,payment,config) {
  ...
  const orderInput={
    ...
    shippingLines:[{title:rate.code==='express'?'Express Delivery':'Standard Delivery',code:rate.code==='express'?'AX_DELHIVERY_EXPRESS':'AX_DELHIVERY_STANDARD',source:'Delhivery',priceSet:shopifyMoney(rate.amount)}],
    ...
    note:`Partial COD · Razorpay advance ₹${session.advance.toFixed(2)} · planned COD balance ₹${session.codBalance.toFixed(2)}.`,
...
export async function preparePartialCod({cartId,pincode,shippingCode,customer},{env=process.env}={}) {
  ...
  const session={
    ...
    pincode:quote.pincode,location:quote.location,cart:quote.cart,rate:{
      code:rate.code,label:rate.label,amount:money(rate.amount),currency:'INR'
    },
...
export async function confirmPartialCod({sessionToken,razorpayOrderId,razorpayPaymentId,razorpaySignature},{env=process.env}={}) {
  ...
  const waybill=await ensureDelhiveryShipment(order,session,payment.id,{env});
  return {
    ...
    advance:session.advance,waybill
  };
}
