...
export async function queryCustomerAccount(config,session,query,variables={},fetchImpl=fetch) {
  ...
  for(let attempt=0;attempt<2;attempt++) {
    const response=await fetchImpl(apiEndpoint,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json',Authorization:current.accessToken},body:JSON.stringify({query,variables}),cache:'no-store',signal:AbortSignal.timeout(10000)});
