...
export async function discoverCustomerAccount(config,fetchImpl=fetch) {
  ...
  const data=await fetchDiscoveryJson(`https://${config.domain}/.well-known/openid-configuration`,fetchImpl,'Customer account service unavailable.',config);
  const endpoints={
...
export async function discoverCustomerAccountApi(config,fetchImpl=fetch) {
  ...
  const data=await fetchDiscoveryJson(`https://${config.domain}/.well-known/customer-account-api`,fetchImpl,'Customer account API unavailable.',config);
