import 'server-only';
// CUSTOMER ACCOUNT OAUTH BOUNDARY
// Before changing this flow for a login bug, verify AX_SITE_ORIGIN plus Shopify callback,
// logout and Customer Account permissions. Configuration mistakes can look like code bugs.
...
const SHOPIFY_USER_AGENT='AX-Mens-Store-Headless/1.0';
const DISCOVERY_TTL_MS=10*60*1000;
const discoveryCaches=new WeakMap();

async function cachedDiscovery(fetchImpl,key,load){
  let cache=discoveryCaches.get(fetchImpl);
  if(!cache){cache=new Map();discoveryCaches.set(fetchImpl,cache);}
  const hit=cache.get(key);
  if(hit && hit.expiresAt>Date.now()) return hit.value;
  try{
    const value=await load();
    cache.set(key,{value,expiresAt:Date.now()+DISCOVERY_TTL_MS});
    return value;
  }catch(error){
    cache.delete(key);
    throw error;
...
function shopifyHeaders(config,headers={}) {
  return {Origin:config.siteOrigin,'User-Agent':SHOPIFY_USER_AGENT,...headers};
}

export function sealAccount(value,secret){return sealEncryptedToken(value,secret,'v1');}

export function unsealAccount(token,secret){
  return unsealEncryptedToken(token,secret,{prefix:'v1',minLength:20,maxLength:12000});
}
...
export function authCompleteUrl(config,returnTo='/account') { const url=new URL('/account/auth-complete',config.siteOrigin);url.searchParams.set('returnTo',safeReturnTo(returnTo));return url.toString(); }
...
async function fetchDiscoveryJson(url,fetchImpl,message,config) {
  const response=await fetchImpl(url,{method:'GET',headers:shopifyHeaders(config,{Accept:'application/json'}),cache:'no-store',signal:AbortSignal.timeout(10000)});
...
export async function discoverCustomerAccount(config,fetchImpl=fetch) {
  ...
  if(overrides.authorizationEndpoint && overrides.tokenEndpoint && overrides.logoutEndpoint) return overrides;
  const data=await cachedDiscovery(fetchImpl,'oauth:'+config.domain,()=>fetchDiscoveryJson(`https://${config.domain}/.well-known/openid-configuration`,fetchImpl,'Customer account service unavailable.',config));
  const endpoints={
...
export async function discoverCustomerAccountApi(config,fetchImpl=fetch) {
  const override=endpoint(config.apiEndpoint);
  if(override) return override;
  const data=await cachedDiscovery(fetchImpl,'api:'+config.domain,()=>fetchDiscoveryJson(`https://${config.domain}/.well-known/customer-account-api`,fetchImpl,'Customer account API unavailable.',config));
  const apiEndpoint=endpoint(data.graphql_api);
  if(!apiEndpoint) throw new Error('Customer account API is not configured.');
  return apiEndpoint;
}
...
async function tokenRequest(config,endpoints,body,fetchImpl) {
  const response=await fetchImpl(endpoints.tokenEndpoint,{method:'POST',headers:shopifyHeaders(config,{Accept:'application/json','Content-Type':'application/x-www-form-urlencoded',Authorization:basicAuth(config)}),body:new URLSearchParams({client_id:config.clientId,...body}),cache:'no-store',signal:AbortSignal.timeout(10000)});
...
export async function queryCustomerAccount(config,session,query,variables={},fetchImpl=fetch) {
  const [endpoints,apiEndpoint]=await Promise.all([
    discoverCustomerAccount(config,fetchImpl),
    discoverCustomerAccountApi(config,fetchImpl)
  ]);
  let current=session;
  for(let attempt=0;attempt<2;attempt++) {
    const response=await fetchImpl(apiEndpoint,{method:'POST',headers:shopifyHeaders(config,{Accept:'application/json','Content-Type':'application/json',Authorization:current.accessToken}),body:JSON.stringify({query,variables}),cache:'no-store',signal:AbortSignal.timeout(10000)});
