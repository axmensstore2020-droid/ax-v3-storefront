...
async function fetchDiscoveryJson(url,fetchImpl,message) {
  const response=await fetchImpl(url,{method:'GET',headers:{Accept:'application/json'},cache:'no-store',signal:AbortSignal.timeout(10000)});
...
export async function discoverCustomerAccount(config,fetchImpl=fetch) {
  ...
  const data=await fetchDiscoveryJson(`https://${config.domain}/.well-known/openid-configuration`,fetchImpl,'Customer account service unavailable.');
  const endpoints={
...
export async function discoverCustomerAccountApi(config,fetchImpl=fetch) {
  ...
  const data=await fetchDiscoveryJson(`https://${config.domain}/.well-known/customer-account-api`,fetchImpl,'Customer account API unavailable.');
...
async function tokenRequest(config,endpoints,body,fetchImpl) {
  const response=await fetchImpl(endpoints.tokenEndpoint,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/x-www-form-urlencoded',Authorization:basicAuth(config)},body:new URLSearchParams({client_id:config.clientId,...body}),cache:'no-store',signal:AbortSignal.timeout(10000)});
...
export async function queryCustomerAccount(config,session,query,variables={},fetchImpl=fetch) {
  ...
  for(let attempt=0;attempt<2;attempt++) {
    const response=await fetchImpl(apiEndpoint,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json',Authorization:`Bearer ${current.accessToken}`},body:JSON.stringify({query,variables}),cache:'no-store',signal:AbortSignal.timeout(10000)});
