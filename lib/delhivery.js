...
function normalizeServiceability(payload,pincode) {
  ...
  for(const row of rows) {
    ...
    const serviceable=['Y','YES','TRUE','1'].includes(prepaid) && !/embargo/i.test(remarks);
    return {
      serviceable,
  ...
  return {serviceable:false,city:'',district:'',stateCode:'',isOda:false,remarks:''};
}
...
function customerDeliveryEstimate({surface,express,isOda=false}={}) {
  ...
  return isOda
    ? {minDays:5,maxDays:10,source:'ax-fallback'}
    : {minDays:3,maxDays:7,source:'ax-fallback'};
}
...
export async function estimateDelhiveryDelivery({destinationPincode,weightGrams,subtotal=0},{env=process.env,fetchImpl=fetch}={}) {
  ...
  if(!Number.isFinite(grams) || grams<1 || grams>100000) {
    return {
      ...
      estimatedDelivery:customerDeliveryEstimate({isOda:location.isOda}),error:''
    };
  }
  ...
  return {
    ...
    estimatedDelivery:customerDeliveryEstimate({surface,express,isOda:location.isOda}),
