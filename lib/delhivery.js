...
export async function quoteDelhiveryRate({originPincode,destinationPincode,weightGrams,mode,paymentMode='Pre-paid'},{env=process.env,fetchImpl=fetch}={}) {
  ...
  try {
    ...
    return quote.amount==null
      ? {configured:true,amount:null,minDays:null,maxDays:null,error:'Delhivery did not return a rate for this service.'}
...
function customerDeliveryEstimate({surface,express}={}) {
  const preferred=surface?.minDays ? surface : express?.minDays ? express : null;
  if(!preferred) return null;
  return {
    minDays:preferred.minDays,
    maxDays:preferred.maxDays ?? preferred.minDays,
    source:'delhivery'
  };
}

export async function estimateDelhiveryDelivery({destinationPincode,weightGrams,subtotal=0},{env=process.env,fetchImpl=fetch}={}) {
  ...
  if(!Number.isFinite(grams) || grams<1 || grams>100000) {
    return {
      ...
      estimatedDelivery:null,error:''
    };
  }
  const originPincode=normalizePincode(env.DELHIVERY_ORIGIN_PIN || '641011');
  if(!originPincode) return {configured:true,serviceable:true,pincode,location,rates:[],needsWeight:false,error:'The pickup pincode is not configured.'};
...
export async function estimateDelhiveryCodDelivery({destinationPincode,weightGrams,subtotal=0},{env=process.env,fetchImpl=fetch}={}) {
  ...
  const [surface,express]=await Promise.all([
    quoteDelhiveryRate({originPincode,destinationPincode:pincode,weightGrams:grams,mode:'S',paymentMode:'COD'},{env,fetchImpl}),
    quoteDelhiveryRate({originPincode,destinationPincode:pincode,weightGrams:grams,mode:'E',paymentMode:'COD'},{env,fetchImpl})
  ]);
  const fee=checkoutFee(env);
  const threshold=configuredFreeShippingThreshold(env);
  const orderSubtotal=Number(subtotal);
...
export async function findDelhiveryShipmentByOrderId(value,{env=process.env,fetchImpl=fetch}={}) {
  ...
  try {
    const response=await fetchImpl(url,{method:'GET',headers:authHeaders(token),cache:'no-store',signal:safeTimeout()});
    if(!response.ok) return {configured:true,shipment:null,error:'Delhivery tracking is temporarily unavailable.'};
    let payload;try {payload=await response.json();} catch {return {configured:true,shipment:null,error:'Delhivery tracking is temporarily unavailable.'};}
    ...
    return {configured:true,shipment:null,error:'Delhivery tracking is temporarily unavailable.'};
  }
}
...
export async function createDelhiveryCodShipment(input,{env=process.env,fetchImpl=fetch}={}) {
  ...
  if(!token || !client || !pickup) return {configured:false,waybill:'',error:'Delhivery COD shipment creation is not configured.'};
  ...
  try {
    ...
    if(!response.ok) return {configured:true,waybill:'',error:'Delhivery could not create the COD shipment.'};
    ...
    return {configured:true,waybill:'',error:remark || 'Delhivery did not return a shipment number.'};
  } catch {
    return {configured:true,waybill:'',error:'Delhivery could not create the COD shipment.'};
  }
}
