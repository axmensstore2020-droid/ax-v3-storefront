...
export async function getDelhiveryCheckoutRates(rateRequest,{env=process.env,fetchImpl=fetch}={}) {
  ...
  const fee=checkoutFee(env),freeStandard=freeShippingEligible(rate,env);
  const rates=[];
  if(surface.amount!=null) rates.push(freeStandard?{
    service_name:'Free Standard Delivery',service_code:'AX_DELHIVERY_STANDARD',total_price:'0',description:'Delhivery Surface · Free shipping',currency:'INR'
  }:carrierRate({
