...
export function configuredFreeShippingThreshold(env=process.env) {
  const value=Number(env.AX_FREE_SHIPPING_THRESHOLD || 0);
  return Number.isFinite(value) && value>0 && value<10000000 ? Math.round(value*100)/100 : 0;
}
...
export async function estimateDelhiveryDelivery({destinationPincode,weightGrams,subtotal=0},{env=process.env,fetchImpl=fetch}={}) {
  ...
  if(!Number.isFinite(grams) || grams<1 || grams>100000) {
    return {configured:true,serviceable:true,pincode,location,rates:[],needsWeight:true,error:''};
  }
  ...
  return {
    ...
    error:rates.length?'':(surface.error || express.error || 'Delivery rates are temporarily unavailable.')
  };
}
