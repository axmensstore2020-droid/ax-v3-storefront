export const MEASUREMENT_SCHEMAS = Object.freeze({
  ...
  shirt: ['chest','shoulder','front_length','sleeve'],
...
export const CIRCUMFERENCE_FIELDS = Object.freeze(['chest','waist','hip','thigh','leg_opening']);
...
const FIELD_ALIASES = new Map([
  ...
  ['front_length','front_length'],['front_body_length','front_length'],
...
export function measurementCategory(product = {}) {
  ...
  if (/\b(jeans?|pants?|trousers?|bottoms?|cargos?|joggers?|chinos?)\b/.test(haystack)) return 'bottom';
...
export function normalizeMeasurementRows(measurements = {}, {unit='',basis='',category=''} = {}) {
  ...
  const allowed = new Set(MEASUREMENT_SCHEMAS[category] || []);
  const rows = {};
  for (const [size,row] of Object.entries(measurements)) {
    ...
    for (const [rawField,value] of Object.entries(row)) {
      const field = canonicalMeasurementField(rawField);
